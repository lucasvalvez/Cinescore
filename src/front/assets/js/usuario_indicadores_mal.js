...
async function carregarFavoritos() {
  ...
  if (localStorage.getItem("emailUsuario") != null) {
    ...
    const nomes = [];
    const quantidades = [];
    const responseTitulosMenosCurtidos = await fetch(`http://localhost:8080/api/feedback/titulosMenosCurtidos`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    const responsePorcentagem = await fetch(`http://localhost:8080/api/feedback/indicadorPorcentagemNegativa`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
