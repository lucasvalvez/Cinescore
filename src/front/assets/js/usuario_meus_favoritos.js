...
document.getElementById('remover').addEventListener('click', async function () {
  const action = await fetch(`http://localhost:8080/api/usuarios/deletar?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json'
    }
...
document.getElementById('btnSalvar').addEventListener('click', async function () {
  ...
  const action = await fetch(`http://localhost:8080/api/usuarios/alterar?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
    },
